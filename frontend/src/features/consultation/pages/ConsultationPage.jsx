...
export default function ConsultationSearch() {
  ...
  const handleSearch = useCallback(async (searchTerm = query, searchDate = date) => {
    const isName = searchTerm.trim() !== "";
    const isDate = searchDate !== "";
  ...
  }, [query, date, handleSearch]);

  const handleKeyDown = (e) => {
    if (e.key === "Enter") handleSearch(query, date);
  };
          ...
          {patients.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {patients.map((p) => (
                ...
                >
                  <div
                    className={`w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${getColor(p.id)}`}
                  >
