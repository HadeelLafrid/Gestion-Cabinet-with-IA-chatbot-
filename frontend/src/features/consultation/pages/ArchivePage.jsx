...
export default function ArchivePage() {
  ...
  const filteredPatients = hasSearched ? archive : [];
  ...
  const handleSearch = useCallback(async (searchTerm = query) => {
    ...
    try {
      ...
      const response = await apiClient.get("/consultations/", {
        params: { patient_search: trimmedQuery },
      });
      ...
      consultations.forEach((consultation, index) => {
        ...
        patientData.consultations.push({
          id: consultation.id,
          rawDate: consultation.consultation_date,
            ...
            filteredPatients.map((patient) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setExpandedId(isExpanded ? null : patient.patientId)
                    }
                    className="w-full flex items-center gap-4 px-6 py-5 hover:bg-gray-50 transition-colors text-left"
                  >
                    <div
                      className={`w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${patient.color}`}
                    >
